...
public class RelarioSubscriptionManager : MonoBehaviour
{
    ...
    private RelarioPayment relarioPayment;
    ...
    private void Start()
    {
        relarioPayment = this.GetComponent<RelarioPayment>();
    ...
    IEnumerator GetSMSPrices()
    {
        ...
        yield return StartCoroutine(GetIP(ipAddress =>
        {
            ...
            StartCoroutine(API.GetSMSPrices(smsHandler.apiKey, ipAddress, (exception, smsPricesResponse) =>
            {
                if (exception != null)
                {
    ...
    private void CheckSubscriptionRewards()
    {
        ...
        if (logRemainingDays)
        {
            ...
            // uIController.UpdateTimerText($"Remaining days for subscription type {subscriptionType} is {remainingDays}");
            // Log the remaining days
            Debug.Log($"Remaining days for subscription type {subscriptionType}: {remainingDays}");
            uIController.UpdateTimerText($"Days  left for subscription type {subscriptionType} is {remainingDays}");
        }
    ...
    public void SkipDays()
    {
        ...
        uIController.UpdateTimerText($"Rewards Granted :: through skipping");
